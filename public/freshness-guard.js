...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      "@media (max-width:640px){#cmr-research{right:8px;left:8px;width:auto;bottom:8px}}",
    ].join("");
  ...
  function ensureResearch() {
    ...
    el.innerHTML = [
      ...
      "</div>",
      "<div id='cmr-panel-setups' class='panel'><div class='muted'>Hypothesis detector — compression ≠ bullish</div><div id='cmr-setups'></div></div>",
    ...
    el.querySelectorAll(".tabs button").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (activeTab === "btc") loadBtc();
        if (activeTab === "traction") loadTraction();
        if (activeTab === "holdings") renderWatch();
      });
    });
  ...
  async function loadSetups() {
    ...
    try {
      const res = await origFetch("/api/early-setups?limit=10&candles=1&timeframe=1h");
      ...
      html += setups.slice(0, 10).map(function (s) {
        ...
        return (
          ...
          " · " + (s.dataConfidence || "") + brLine +
          "</div><div class='muted'>" + ((s.evidence && s.evidence[0]) || "") + "</div></span></div>"
        );
      }).join("");
      box.innerHTML = html;
    } catch (_) {
