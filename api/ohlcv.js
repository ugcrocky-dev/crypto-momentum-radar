...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.end(JSON.stringify({
      ...
      metadata: { pair: result.pair, timeframe, approximate: Boolean(result.approximate), providers: OHLCV_PROVIDERS, fetchedAt: new Date().toISOString() },
      data: { candles: result.candles, freshness, coverage: { count: result.candles.length, missingVolume: result.candles.filter((c) => c.volume == null).length, note: "Volume is exchange/base volume or market total_volume - not trade counts." } },
    }));
  } catch (err) {
    res.statusCode = 503;
