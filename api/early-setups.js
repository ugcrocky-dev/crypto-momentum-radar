...
export default async function handler(req, res) {
  ...
  const sourceGeneratedAt = momentum?.data?.sourceGeneratedAt || momentum?.data?.generatedAt || null;
  const freshness = computeFreshness({ sourceGeneratedAt, nowMs, cadence: MAIN_CADENCE });
  ...
  for (const row of rows) {
    ...
    if (withCandles && row.symbol) {
      try {
        const ohlcv = await fetchCandles({ symbol: row.symbol, coinId: row.id || null, timeframe, limit: 120 });
        setup = buildEarlySetup({
          symbol: row.symbol, name: row.name, id: row.id || row.symbol, timeframe,
          candles: ohlcv.candles, momentumRow: row, btcShortExcessPp: shortExcess, nowMs,
          provisional: Boolean(ohlcv.approximate),
        });
        setup.ohlcv = { provider: ohlcv.provider, pair: ohlcv.pair, approximate: Boolean(ohlcv.approximate), candleCount: ohlcv.candles.length };
      } catch (err) {
  ...
  res.end(JSON.stringify({
    source: "early-setups",
    metadata: { timeframe, limit, withCandles, fetchedAt: new Date(nowMs).toISOString(), momentumSource: momentum?.source || null },
    data: {
      generatedAt: new Date(nowMs).toISOString(),
      sourceGeneratedAt,
      freshness: {
        status: freshness.status, ageHours: freshness.ageHours, ageMinutes: freshness.ageMinutes,
        sourceGeneratedAt: freshness.sourceGeneratedAt, evaluatedAt: freshness.evaluatedAt, actionable: freshness.actionable,
        ...(freshness.reason ? { reason: freshness.reason } : {}),
      },
      setups,
      methodology: {
        states: ["Coiling", "Igniting", "Confirmed", "Failed", "Expired"],
        note: "Compression is direction-neutral. Hypothesis weights — not proven. Not trade advice.",
      },
    },
  }));
}
