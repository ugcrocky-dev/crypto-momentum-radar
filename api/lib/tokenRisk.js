...
const CACHE_PREFIX = "cmr:token-risk:v1:";
...
export async function enrichRowsWithRisk(rows, { maxChecks = MAX_CHECKS_PER_PASS } = {}) {
  if (!Array.isArray(rows) || !rows.length) {
    return { rows: rows || [], riskMeta: { checked: 0, risky: 0, cached: 0, skipped: 0 } };
  }

  const meta = { checked: 0, risky: 0, cached: 0, skipped: 0, errors: 0 };
