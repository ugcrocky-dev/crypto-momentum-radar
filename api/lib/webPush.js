/**
 * Web Push helpers. Optional — only active when VAPID env is set AND web-push
 * can be loaded. ntfy is the zero-dependency phone channel used by default.
 */
...
async function loadWebPush() {
  if (webpushLoadAttempted) return webpushMod;
  webpushLoadAttempted = true;
  try {
    webpushMod = (await import("web-push")).default || (await import("web-push"));
  } catch {
    webpushMod = null;
  }
  return webpushMod;
...
async function applyVapid() {
  ...
  if (!webpush?.setVapidDetails) return null;
...
export async function sendWebPushFomoAlert(alert) {
  const webpush = await applyVapid();
  if (!webpush) {
