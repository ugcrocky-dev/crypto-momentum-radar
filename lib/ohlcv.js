...
export async function fetchCoinGeckoApprox(coinId, timeframe = "1h", limit = 100) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        lastErr = Object.assign(new Error("provider_rate_limited"), { code: 429 });
  ...
  throw lastErr || new Error("provider_rate_limited");
}
...
export async function fetchCandles({ symbol, coinId, timeframe = "1h", limit = 100 } = {}) {
  const errors = [];
  // On Vercel / when Binance is geo-blocked, prefer CoinGecko first to avoid
  // burning the free-tier budget on doomed Binance calls + CG retries.
  const preferCg =
    process.env.OHLCV_PREFER_COINGECKO === "1" ||
    Boolean(process.env.VERCEL) ||
    Boolean(binanceBlockedReason);

  const tryBinance = async () => {
    if (!symbol || binanceBlockedReason) return null;
    try {
  ...
  const tryCg = async () => {
    ...
    if (!resolvedCoinId) {
      if (symbol) errors.push("coingecko_id_unresolved");
      return null;
    }
    try {
  ...
  if (!preferCg) {
    const binance = await tryBinance();
    if (binance) return binance;
    const cg = await tryCg();
    if (cg) return cg;
  } else {
    const cg = await tryCg();
    if (cg) return cg;
